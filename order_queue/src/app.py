...
class OrderQueueService(queue_grpc.OrderQueueServicer):
    ...
    def Enqueue(self, request, context):
        with self._lock:
            ...
            logger.info(f"[{request.order_id}] 📨 Enqueued with priority {priority} "
                        f"(premium={request.is_premium}, shipping='{request.shipping_method}', items={request.num_items})")
    ...
    def Dequeue(self, request, context):
        with self._lock:
            if self._queue:
                priority, order_id = heapq.heappop(self._queue)
                logger.info(f"[{order_id}] ✅ Dequeued for execution | priority={priority}")
